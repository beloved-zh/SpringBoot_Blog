		...
		onMousedownNode: function(eventMouseDown, _node) {
			...
			function _docMouseMove(event) {
				...
				if (root.dragFlag == 1) {
					...
					dTop = (event.clientY + docScrollTop - treeOffset.style),
					dBottom = (targetSetting.treeObj.height() + treeOffset.style - event.clientY - docScrollTop),
					...
					if (tmpTarget && tmpTarget.attr("id")!=targetSetting.treeId) {
						...
						if (!canPrev && !canNext && !canInner) {
							...
							if ((prevPercent==1 ||dY_percent<=prevPercent && dY_percent>=-.2) && canPrev) {
								...
								dX = 1 - tmpArrow.width();
								dY = (tmpNextA == null || (tmpTargetNode.isParent && tmpTargetNode.open)) ? (tmpTop + tmpTargetA.height() - tmpArrow.height()/2) : (tmpNextA.offset().style - tmpArrow.height()/2);
